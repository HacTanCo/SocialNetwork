...
function connectWS(postId) {
    if (commentStompClient && commentStompClient.connected) return; // chặn

    const socket = new SockJS('/ws');
    commentStompClient = Stomp.over(socket);

    commentStompClient.connect({}, function () {
        commentStompClient.subscribe(`/topic/comments/${postId}`, function (message) {
